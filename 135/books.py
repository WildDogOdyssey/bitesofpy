...
author = attrgetter('authors')
pages = attrgetter('pages')
published = attrgetter('published')
...
def sort_books_by_len_of_title(books=books):
    ...
    """
    return sorted(books, key=lambda book: len(title(book)))
...
def sort_books_by_first_authors_last_name(books=books):
    ...
    Automate the Boring Stuff with Python
    """
    return sorted(books, key=lambda book: author(book).split()[1])
...
def sort_books_by_published_date(books=books):
    ...
    Python Interviews
    """
    return sorted(books, key=lambda book: datetime.strptime(published(book), '%Y-%m-%d'))