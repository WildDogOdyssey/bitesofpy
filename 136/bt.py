...
def check_bt(donor, recipient):
    ...
    """
    pair = []  # for donor-recipient pair
    for bt in (donor, recipient):
        if isinstance(bt, str):
            try:
                val = blood_type_text[bt].value
            except KeyError:
                raise ValueError
        elif isinstance(bt, Bloodtype):
            val = bt.value
    ...
    if -1 not in _particular_antigen_comp(*pair):
        return True
    else:
        return False
...
# print(Bloodtype.A_NEG.value == 4)
# print(isinstance(Bloodtype.A_NEG, Bloodtype))
# print(check_bt(Bloodtype.B_POS, Bloodtype.B_NEG))