...
def filter_tweets_on_polarity(tweets, keep_positive=True):
    ...
    return list(filter(lambda x:
                       x.polarity > 0 if keep_positive else x.polarity < 0,
                       tweets))

# print(filter_tweets_on_polarity(tweets))
...
def order_tweets_by_polarity(tweets, positive_highest=True):
    ...
    return list(sorted(tweets, key=lambda x: x.polarity, reverse=positive_highest))

# print(order_tweets_by_polarity(tweets))