...
class BiteStats:
    ...
    def number_bites_accessed(self) -> int:
        ...
        return len(set(bite['bite'] for bite in self.rows))

    @property
    def number_bites_resolved(self) -> int:
        """Get the number of unique Bites resolved (completed=True)"""
        return len(set(bite['bite'] for bite in self.rows if bite['completed']=='True'))

    @property
    def number_users_active(self) -> int:
        """Get the number of unique users in the data set"""
        return len(set(bite['user'] for bite in self.rows))

    @property
    def number_users_solving_bites(self) -> int:
        """Get the number of unique users that resolved
          one or more Bites"""
        return len(set(bite['user'] for bite in self.rows if bite['completed']=='True'))

    @property
    def top_bite_by_number_of_clicks(self) -> str:
        """Get the Bite that got accessed the most
          (= in most rows)"""
        cnt = Counter([bite['bite'] for bite in self.rows])
        return int(cnt.most_common(1)[0][0])

    @property
    def top_user_by_bites_completed(self) -> str:
        """Get the user that completed the most Bites"""
        cnt = Counter([bite['user'] for bite in self.rows if bite['completed']=='True'])
