...
# gp_handle = 'https://pkgstore.datahub.io/core/gold-prices/annual_csv/data/343f626dd4f7bae813cfaac23fccd1bc/annual_csv.csv'
gold_prices = """
...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    """
    gold_df = load_df(gold_prices)
    gold_df['Change'] = gold_df.Price.diff()

    min_delta = gold_df.Change.min()
    max_delta = gold_df.Change.max()

    min_year = gold_df[gold_df.Change == min_delta].iloc[0, 0]
    max_year = gold_df[gold_df.Change == max_delta].iloc[0, 0]
    return (int(min_year[:4]), int(max_year[:4]))

@lru_cache()
def load_df(string):
    """Helper function to load a string into a data frame.
    """
    df = pd.DataFrame([row.split(',') for row in string.split()])
    df.columns = ['Date', 'Price']
    df.Price = df.Price.astype('float32')
    return df

# print(load_df(gold_prices))
