...
from functools import lru_cache  # extra
...
#         return [line.rstrip() for line in f.readlines()]
    ...
    # print(page.text)
    return [line.rstrip() for line in page.text.splitlines()]
...
def filter_killed_mutants(mutpy_output: list = None) -> list:
    ...
    for i, line in enumerate(mutpy_output):
        if '- [#' in line:
            start = i
        elif 's] killed by' in line:
            chucks.append((start, i))
        elif 's] incompetent' in line:
            chucks.append((start, i))

    for start, end in reversed(chucks):
        del mutpy_output[(start + 1):end]
